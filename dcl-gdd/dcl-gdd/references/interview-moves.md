...
1. **One decision per message** (hard rule 2 — that is its home). Never a batch, never "also, quick
   follow-up". A list lets the owner
...
- **Rubric cosplay** — writing the words the rubric rewards without the design underneath. This is
  ...
  are drawn from the rubric, so an owner can agree their way to a document that reads well to a
  reviewer without deciding anything.
