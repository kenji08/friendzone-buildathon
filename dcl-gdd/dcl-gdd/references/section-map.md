...
"Never deferred" binds **you**, not the owner — the card's "skip" promise holds here too. If they
skip the core loop anyway, honour it: go where they pointed, park nearly everything downstream as
`[OPEN: waiting on the core loop]`, and say the price in one line — *"sure — most later sections
lean on the loop, so I'll park what depends on it until we swing back"*. Never push the answer out
of them.
...
- **But the research belongs in the question, not in the table.** The template asks for what the
  *owner* observed, so the rows get written from what they say back; a nod to your summary is
  ...
  alternatives upgrades it to `[agent-choices ✓]` instead). If they have not
...
It carries the most weight in review, so do not let it be answered quickly — **that is a note to
you, not a line to say aloud.** To the owner this is where a promise gets specific, not an exam.
...
- **Five messages for nine rows** — the rows group, and the count rule at the top of this file
  ...
  **meeting and recognition** (strangers to a group in 30 seconds, where a name is first learned,
  ...
  maximum — four numbers, one decision) · **robustness** (drop-in / drop-out — load-bearing, so it
  gets a message of its own; in a list the colour rows would answer for it) · **colour** (the
...
- **Park:** most input and performance claims are mobile-sensitive — mark them so in the log
  ...
  *… — mobile pending* until the mobile rung is settled — the QR pass costs minutes and is offered
  at core-loop stage close (canon: [hypothesis-log.md](hypothesis-log.md)).
...
- **One genuinely new question — the requested round:** **v0** (vertical slice) or **v1** (4-week
  ...
  a design whose fun is not yet found and whose evidence is a greybox usually means finishing
  find-the-fun first — the v0 bar already expects the fun *found*, and an honest §0 says what
  evidence exists today; a
