...
Creator Hub costs minutes (offered at core-loop stage close —
[stages-and-gates.md](stages-and-gates.md), What happens at a gate). The program resolves the remaining
...
- So every verdict in the Hypothesis Log carries `tested on: desktop / mobile`. A
  **mobile-sensitive** hypothesis still closes with a terminal suffix at its verdict; the index
  ...
  program allows (canon: [hypothesis-log.md](hypothesis-log.md)).
...
adapt it". "Out loud" names the content, not the register — a note to you: to the owner it is
*"this verb fights touch — worth deciding now whether the verb changes or the platform does"*, a
choice put on the table, never a verdict.
...
- "The scene holds 30 fps on the named mobile device at the v1 tested maximum" — expensive, and
  worth an early cheap proxy (asset budget arithmetic) before the real measurement.
