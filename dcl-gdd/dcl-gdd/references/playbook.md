...
hours arithmetic must survive multiplication: stated hours × 4 weeks against the week plan
(development sits in weeks 1–4 — §9 below). The light
...
**Calibration — and what the numbers actually measure** *(traced 2026-08-28)*. **This block is the
...
[review-lens.md](review-lens.md)'s **Calibration** — and they
...
program-level target is >20%. Both bars apply only once the view holds at least **ten eligible new
users** with a completed window — below that the verdict is *insufficient evidence*, not a fail.
Do **not** equate these with mobile-F2P "D7 retention", which counts
...
portable experiences are not supported**, so a core loop resting on them cannot ship — this is
exactly the platform-feasibility preflight row, P9 in [review-lens.md](review-lens.md), and
still a hard stop worth raising the moment it appears.
