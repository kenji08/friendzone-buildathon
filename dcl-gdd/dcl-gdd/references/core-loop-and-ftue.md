...
Smells, not bans: signs with instructions, a tutorial, a tutorial NPC, UI narration ("you click the
button"), and lore before the first action. The template's bar is a scene that teaches **without
mandatory reading**, and it allows the **shortest necessary hint**; the first-use-clarity preflight
(P3) fails on **incomprehension, not the presence of text**. Each item on the list is a signal the
scene is not teaching itself yet — fix the frame first, and let whatever hint survives shrink to the
shortest necessary one.
