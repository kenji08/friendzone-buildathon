...
and saying so now is the most useful thing the document does. "Fails in week 3" is a note to you,
not a line to say aloud — the owner gets the arithmetic itself, *"240 hours of content against 120
available — what should survive?"*: the numbers carry the bad news, so the plan is the problem,
never the person. The old six-week framing hid this: the
...
the >20% D7 program target (canon: playbook §4; both bars apply only from ten eligible new users
with a completed window — below that it is insufficient evidence, not a fail), the Discoverability
Bar. The dashboard already tracks return — **do not
