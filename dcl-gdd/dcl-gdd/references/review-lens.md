...
*(No rubric here: the programme retired numeric gates and scored totals, and so has this file.)*
...
- **Never say any of this out loud in these words.** "P4", "preflight", "status" and the rest are
  yours (SKILL.md hard rule 7). To the owner it is *"Why Players Come Back is the thin one — here is
  the cheapest way to thicken it"*.
...
`reviewer-checklist.md` (a citation, not a path — the file is not bundled). The ones that earn their
keep mid-interview:
...
- **Retention numbers.** The programme's figure is the dashboard's **first-week return** — the share
  of eligible new players who come back at least once during days 1–7. The v1 floor is >10% and
  ...
  *(Traced 2026-08-28; provenance in [playbook.md](playbook.md) §4, which is the canon for these
  numbers.)*
