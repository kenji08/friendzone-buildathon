...
| **2 · Pitch** | core loop playable | + §4 in full · §5 social · §8 audience rows · §9 as a skeleton · §0 TL;DR (§0 minus the stage-3 fields below) | Would this hold a player past Day 1? |
...
interview and offer `dcl-prototype`. Fun first, build first. That register is a note to you, not a
line to say aloud — to the owner it is *"let's make it provable — a small greybox would give us
something to link"*, never a doubt that the hand-test happened.
...
4. Say what just became solid, and what is worth designing next — and offer the natural pause: a gate
...
5. Report **harvest coverage** against the stage table above, in one line — *"concept stage closed —
   Player Promise and Core Loop are real; Why Players Come Back, Social by Design and the audience
   fields aren't written yet; about five minutes of pulling from your repo would fill some. Want
   it?"* — and offer it. Counting is mandatory for
