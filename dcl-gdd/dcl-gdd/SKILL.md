...
description: "Turn a Decentraland experience idea — vague, half-written, or already documented — into a Creator Success proposal (a short `design/gdd.md`) that survives review, by interviewing the design owner one question at a time and parking every untestable claim as a falsifiable hypothesis. Entry states: adapt an existing GDD to the program format, solidify a vague idea, resume a half-written one, pick up parked hypotheses, revise after reviewer feedback, or harvest a built scene's repo into the document. Say \"doc-only\" in the argument for a document-only session. Use when someone is writing, adapting, scoring or revising a Creator Success / DCL funding proposal, a short GDD, a concept one-pager or a pitch deck for a Decentraland World; when they ask why a proposal would fail review; or when a design claim about retention, core loop, first 10 seconds, social design, mobile touch or scope needs pressure-testing. Pairs with `dcl-prototype`, which tests the hypotheses this skill parks."
---
...
1. **Never write an answer nobody gave.** Four markers carry the honesty layer, and you never quietly
   ...
   - `[agent-decided]` — you chose on their behalf. Taste is theirs; if they did not say it, mark it.
     ...
     `[agent-choices ✓]` — they picked from the choices you offered: the wording is still yours,
     the decision is now theirs, and at submission these are signed off as a group instead of one
     by one. Hard rule 2 draws the line between a real acceptance and a bare "yes".
...
2. **One decision per message,** with your own recommended answer and the reasoning attached.
   ...
   list lets an owner answer the easy part and skip the load-bearing one. A human reaction line is
   free; the rule counts decisions, not sentences.
   ...
   consequence you spelled out**, has decided — that earns `[agent-choices ✓]`, still
   ...
   skill, so recommending one and hearing "yes" reads well to a reviewer while deciding nothing.
3. **Word caps are the design tool, not bureaucracy.** 25 words for the promise, 180 words for the
   ...
   comparables, at most 3 examples anywhere — the canonical full list is the phase-3 conformance
   count. If it does not fit, the design is not decided yet — say
...
7. **Write in simple, direct English (B2).** The register of the template. The experience must work
   ...
   - **The machinery stays backstage.** Phase numbers, step letters, route names, hard-rule numbers,
     preflight IDs (`P4`), reference filenames, "grown document", "cascade", "gate", "harvest" —
...
- the **phase-0 handshake asks directly**, when no argument set it: *"just the document, or mixing
  in some building and testing as we go?"* — one bundled, explicit question.
...
- All jump offers stay silent: hard rule 6, the empty-disk offer in phase 0, the phase-5 invitation,
  ...
  stage that cannot close on words is a fact worth stating, not a reason to break the mode. The
  gate's mobile QR pass stays silent the same way — it is a test detour, not a document step.
...
- **One sanctioned exception.** When the document is written out to **the current stage's section
  set** (`[OPEN]` resolved or converted to `TBD:`; `TBD:`, `[HYPOTHESIS]`, `[agent-decided]` and
  `[agent-choices ✓]` may remain) and only
...
- **End the session** with the state of the document and the next document step, plus one
  informational line about the log — *"8 hypotheses, riskiest: the Sunday reset hook (H1-03)"* —
...
- **A section appears with its first answer; a field appears with its content.** Content means an
  answer, a `TBD:`, an `[OPEN]`, a `[HYPOTHESIS]` or an `[agent-decided]` (upgraded:
  `[agent-choices ✓]`) — nothing else. A field can
  ...
  invisible. One
  exception: a **fixed-row table** — any table whose rows the template names *with content*, not
  just numbers — appears
  ...
  §3's numbers table, §4.2's progression, §4.3's two chosen hooks, §5's social rows, §8's
  comparables, §9's four weeks.
...
> Doc: `▓▓▓▓░░░░░░` · Player Promise → Core Loop done · Why Players Come Back in progress · ⚑ submission bar: a little of Social by Design left · full doc: Social by Design → 4 Week Plan

Show it at three moments: **in the phase-0 handshake**, **after every section write** (hard rule 4
already has you in the file), and **whenever the owner asks**. At its first showing, add one line
for the owner: *"the flag marks the point where the document is already submittable."* It also
lives in the document's
...
- **The submission bar** — the template's own bolded sentence: *nails Sections 1–3, has at least a
  ...
  deletion walk-through. One question survives the shortcut: Step K's requested round (and the
  header contact), asked before the `TBD:` sweep. Unchecked self-check boxes are the normal state
  of an honest submission, never a blocker.
...
it needs for the phrases that actually give the owner control.
...
too: offer graduation for idea lines that have matured
([references/hypothesis-log.md](references/hypothesis-log.md)), and re-sort anything `dcl-prototype`
parked while building. Treat the invocation argument (*"here's our doc"*,
*"we stopped at retention"*, *"reviewer asked two questions about the return section"*) as a
**hint you verify against
...
Then run the **P1–P9 preflight** from [references/review-lens.md](references/review-lens.md)
...
Close the phase by **announcing your classification in one sentence** and getting a yes — to the
owner in plain words, sections by name, never a route name: *"Sounds like we're growing a fresh
document from your idea — I'll start with the loop itself."*
...
document has parked ones from Step B onward. **Full is a field-level test, never a heading count** —
a non-linearly grown file has every heading long before it is done: full means every section's
*fields* are answered, `TBD:` or deliberately deleted; any absent field or unresolved
`[OPEN]`/`[agent-decided]` → Resume. Full in that sense, only untested hypotheses left → Verify.

**Verify against Revise:** feedback in hand → Revise wins; Verify is the no-feedback state.
...
it, the session-mode question asked directly: *just the document, or mixing in some building and
testing as we go?*).
...
the hypothesis with the cheapest killing test — risk breaks the tie — and hand off to
`dcl-prototype`. Do not
...
- **continue in the order I recommend** — name the next step and, in one line, why it comes next;
...
session, and the owner may not remember they can steer; the full card returns on "help". The
parked hypotheses get **one informational line** — *"8 parked, riskiest: the Sunday reset hook
(H1-03)"* —
...
- **Quote the question budget for the current stage only.** Stage 1 is ten to fifteen questions for
  an idea from scratch, six to eight when a build has already answered most of them. "Question 5 of
  ~30" tells the owner they now live in an interview; later stages only happen if the design earns
  them.
...
submission bar** (for *go full* — when the document completes), followed by the deletion
walk-through whenever the owner is actually submitting.
...
3. The **P1–P9 preflight again**, now over the draft rather than over the input
   ([references/review-lens.md](references/review-lens.md)). Phase 0 ran it over whatever
...
4. A **conformance count** — mechanical, no judgement, straight against the template: 25 words on
   ...
   · and every field the template labels with a sentence/line limit.
...
5. An honest read of the **review areas** ([references/review-lens.md](references/review-lens.md))
...
- `[HYPOTHESIS]` — rewrite the claim in **design-intent voice**: *"the Sunday reset is the intended
  ...
  is exactly what a proposal is for. Only a claim asserting a *measurement* — a number, an
  *already observed* player behaviour, one claimed as evidence — becomes `TBD:` instead, because
  that one pretends to evidence; a template-shaped predictive sentence stays in design-intent
  voice. The falsifiable prediction itself stays in the Hypothesis Log.
- `[agent-decided]` — needs an explicit owner yes: **`[agent-choices ✓]` ones as one group** (they already
  said yes in conversation — read the list back, one yes covers it; anything they hesitate on drops
  back to one-by-one), **plain ones grouped by section**, each read back with the consequence it
  carries; one by one only for the ones that draw a hesitation. Either way this yes is a sign-off
  on a decision already on the table, not a new decision — hard rule 2's specialise-test applies at
  the moment of deciding, not here, so a plain "yes" clears an item. This is the marker that matters most at
...
- `[OPEN]` — the owner chooses per item: answer now, `TBD:` (the dependency tail becomes the plan;
  postponed becomes `TBD: not discussed yet`), or the field goes.
...
stamped with version and date, and say what just became solid, and what is worth designing next.
...
*"concept stage closed — Player Promise and Core Loop are real; Why Players Come Back, Social by
Design and the audience fields aren't written yet; about five minutes of pulling from your repo
would fill some. Want it?"* — and offer
...
Never end with "start building". End with **the open hypothesis whose killing test is cheapest —
risk breaks the tie** — named by its claim in plain words with the ID in brackets (*"the Sunday
reset hook (H1-03)"*), and the invitation to run `dcl-prototype` on it. In a
...
1. **A separate session** (*recommended for a real build*) — the owner opens a new window in the
   ...
   > Working folder: `<the folder we are in>`. Run `dcl-prototype` with: "experiment `<H-ID>` — `<one
